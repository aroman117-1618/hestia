...
public class AuthService: ObservableObject {
    ...
    @Published public private(set) var isDeviceRegistered: Bool = false
    ...
    public var autoLockTimeout: TimeInterval = 30 * 60
    ...
    // Cached API client for registration
    ...
    public func checkAuthenticationTimeout() -> Bool {
    ...
    /// Register using an invite token from QR code onboarding
    public func registerWithInvite(inviteToken: String) async throws -> String {
        let deviceName = APIClient.deviceInfo?.deviceName ?? "Unknown"
        let deviceType = APIClient.deviceInfo?.deviceType ?? "Unknown"

        let response = try await registrationClient.registerWithInvite(
