...
6. Run `source .venv/bin/activate && python -m pytest --tb=short -q 2>&1 | tail -10` to verify test baseline
7. Run `xcodebuild -scheme HestiaWorkspace -destination 'platform=macOS' build 2>&1 | tail -5` to verify macOS build health
...
**macOS build**: Clean / Failing ([error])
