...
- **Cloud API fallback**: Adds complexity, latency, and dependency. Deferred to future version.
...
- Mitigation: Document which task types are out of scope; consider cloud fallback in v2
...
#### Context
Hestia needs external APIs for capabilities beyond local model (weather, web search, cloud LLM fallback). Must balance capability with cost and security.

#### Decision
**v1.0 External Services**:
- **Cloud LLM Fallback**: Anthropic API (Claude) when Mixtral 8x7B insufficient
...
#### Alternatives Considered
- **All-local (no external APIs)**: Limits capability (no weather, no web search, no cloud fallback)
...
- Positive: Cloud fallback ensures Hestia can handle complex tasks beyond Mixtral's capability
...
- Consider upgrading to larger context model (e.g., Claude API) for complex tasks if Mixtral hits limits
- Future: Implement automatic escalation to cloud model when local context insufficient
